...
기존 e-book 앱에는 책 읽어주기 기능이 이미 있지만, 책을 찾는 과정에서는 메뉴, 권한 안내, 책장 정보, 광고성 문구처럼 필요 없는 정보까지 음성으로 듣게 되는 문제가 있습니다. 이 앱은 사용자가 필요한 책 후보에 바로 접근할 수 있도록 도서 API 검색, 접근성 트리 기반 텍스트 추출, 화면 OCR 보조 기능을 결합합니다.
...
- 접근성 트리 기반 텍스트 추출
- 접근성 텍스트가 부족할 때만 화면 캡처 OCR로 보조
- 검색/목록 화면에서 책 제목, 저자, 출판사, 가격 후보를 추출하는 앱별 필터
...
앱 첫 화면의 버튼을 위에서부터 눌러 권한을 켜면 됩니다. 기본 동작은 접근성 트리에서 텍스트를 먼저 읽는 방식이며, 화면 텍스트가 부족한 경우에만 화면 캡처 권한을 사용합니다.
...
5. 오버레이 결과 앞의 출처 라벨을 확인합니다.
   - `[접근성 트리]`: 화면 UI가 제공한 접근성 텍스트를 읽은 결과입니다.
   - `[OCR]`: 접근성 텍스트가 부족해 화면 캡처 OCR로 읽은 결과입니다.
6. 결과가 메뉴 전체가 아니라 책 제목, 저자, 출판사, 가격 중심으로 표시되는지 확인합니다.

## 현재 지원 범위

- YES24 eBook: 접근성 텍스트 우선, OCR 보조 필터 지원
- 교보문고: 접근성 텍스트 우선, OCR 보조 필터 지원
- 알라딘 eBook: API 검색 및 접근성 텍스트 우선, OCR 보조 필터 지원
